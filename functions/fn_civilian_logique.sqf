...
// --- MAIN LOOP ---
// Exécute sur le serveur uniquement (ou host en solo)
...
[] spawn {
    ...
    while {true} do {
        ...
        if (count _poolTemplates > 0 && count _activeAgents < CIV_MaxCivilians) then {
            {
                private _player = _x;
                ...
                {
                    if (_player distance _x < 500) exitWith {_inZone = true};
                } forEach _presenceZones;
                ...
                if (_inZone) then {
                    ...
                    if (count _validSpawns > 0) then {
                         // OPTIMISATION: Spawn jusqu'à 5 civils par boucle (au lieu de 1)
                         private _toSpawn = (CIV_MaxCivilians - count _activeAgents) min 5;
                         for "_j" from 1 to _toSpawn do {
                             if (count _activeAgents < CIV_MaxCivilians) then {
                                 ...
                                 private _newAgent = [_template, _spawnPos] call CIV_fnc_spawnAgent;
        ...
        for "_i" from (count _activeAgents - 1) to 0 step -1 do {
            ...
            private _far = true;
            {
                if (_a distance _x < CIV_DespawnRadius) exitWith {_far = false};
            } forEach _players;
        ...
        sleep 1; // Check plus fréquent (1s) pour spawn réactif
    };
};