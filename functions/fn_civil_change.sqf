/*
    FONCTION: fn_civil_change
    DESCRIPTION: Convertit périodiquement des civils en insurgés OPFOR
    
    - Vérifie toutes les 100 à 400 secondes (aléatoire) s'il y a 3+ civils à plus de 50m du joueur
    - Sélectionne 3 civils, les convertit en OPFOR (même tenue)
    - Leur donne: sac coyote (B_Messenger_Coyote_F) + AKM + munitions
    - Ils attaquent les OPFOR les plus proches
*/
...
CIVIL_CHANGE_fnc_convertToInsurgent = {
    ...
    _insurgent addWeapon "arifle_AKM_F";
    _insurgent addPrimaryWeaponItem "30Rnd_762x39_Mag_F"; // Chargeur dans l'arme
    
    // Ajouter des munitions supplémentaires dans le sac
    for "_i" from 1 to 6 do {
        _insurgent addItemToBackpack "30Rnd_762x39_Mag_F";
    };
...
[] spawn {
    ...
    while {true} do {
        ...
        if (count _players > 0) then {
            ...
            // Si on a au moins 3 civils éligibles
            if (count _allCivilians >= CIVIL_CHANGE_RequiredCount) then {
                // Mélanger et sélectionner 3 civils
                ...
                ["Conversion terminée - 3 insurgés actifs"] call CIVIL_CHANGE_fnc_log;
            } else {
